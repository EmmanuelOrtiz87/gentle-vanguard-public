...
try {
    ...
    const outputConfig = JSON.parse(readFileSync(join(ROOT, 'config/output-compression.json'), 'utf-8'));
...
try {
    ...
    const testResult = spawnSync('npx', ['tsx', 'src/token-budget-guard.ts', '-Mode', 'check', '-Task', 'validation', '-Risk', 'low', '-Quiet'], {
        cwd: ROOT,
        stdio: 'pipe',
        encoding: 'utf-8'
    });
    
    if (testResult.status === 0 || testResult.status === 2) { // 2 = hard limit reached (permitted)
