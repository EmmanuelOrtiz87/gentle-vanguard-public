        ...
        try {
            if (metric.file.includes('stats')) {
                ...
                if (metrics.length > 0) {
                    ...
                    console.log(`   - Tokens totales: ${last.metrics.totalTokensIn} in / ${last.metrics.totalTokensOut} out`);
                    console.log(`   - Ahorro: ${last.metrics.totalSavings} tokens (${last.metrics.totalReduction.toFixed(1)}%)`);
                }
            } else {
