...
async function calculateHealthScore(metrics: PerformanceMetrics): Promise<{ score: number; components: Record<string, number> }> {
  ...
  const score = Object.values(components).reduce((a, b) => a + b, 0) / Object.keys(components).length;
