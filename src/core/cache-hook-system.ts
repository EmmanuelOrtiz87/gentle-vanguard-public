...
function generateCacheKey(input: string, context: string = ''): string {
  ...
  const normalized = input
    ...
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 400); // Más largo para mejor matching
...
// ─── Buscar en Cache ───────────────────────────────────────────────────────
export function checkCache(input: string, context: string = ''): { 
  hit: boolean; 
  response?: string; 
  tokensSaved?: number;
} {
  ...
  try {
    ...
    if (cached) {
      ...
      if (CACHE_CONFIG.logHits) {
        log(`[HIT] Cache hit! Saved ${cached.tokensSaved} tokens (hit rate: ${STATS.hitRate.toFixed(1)}%)`);
      }
...
function flushCache(): void {
  ...
  log(`[SHUTDOWN] Final stats: ${STATS.hitRate.toFixed(1)}% hit rate, ${STATS.tokensSaved} tokens saved`);
}
