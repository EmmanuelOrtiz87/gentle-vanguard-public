...
function main(): void {
  ...
  if (needNewFile) {
    const headers =
      'SessionID,Date,User,Project,TokensIn,TokensOut,SkillsUsed,SystemsTriggered,ActionsPerformed,Outcome,IssuesFound,Duration(min),Cost(USD),Notes';
      ...
      try {
        ...
        try {
          if (fs.existsSync(telemetryDir)) {
            const telemetryFiles = fs
              .readdirSync(telemetryDir)
              .filter((f) => f.includes(sessionId) && f.endsWith('.json'));
            if (telemetryFiles.length > 0) {
              const telemetryRaw = fs.readFileSync(
                path.join(telemetryDir, telemetryFiles[0]),
                'utf-8',
              );
        ...
        if (engramObservations.length > 0) {
          const sessionObs = engramObservations.filter((obs) => {
            ...
            return (
              obsSessionId.includes(sessionId) ||
              obsSession.includes(sessionId) ||
              obsTitle.includes(sessionId)
            );
          });
            ...
            const skillTitles = [
              ...new Set(
                sessionObs
                  .filter(
                    (obs) =>
                      obs.type === 'skill' || (obs.title && /skill|Skill/.test(String(obs.title))),
                  )
                ...
                sessionObs
                  .filter(
                    (obs) =>
                      ...
                      obs.type === 'manual' ||
                      (obs.title &&
                        /Fixed|Created|Updated|Implemented|Validated/.test(String(obs.title))),
                  )
                  .map((obs) => String(obs.title))
                  .filter(Boolean),
              ),
