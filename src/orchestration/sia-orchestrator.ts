...
function cmdSaveTarget(
  ...
  const dir = sessionDir(sid, outputDir);
  const state = readJson<SiaState>(join(dir, 'state.json'));
  const dest = join(dir, `target-${state.iteration}.ts`);

  if (targetPath) {
    if (!targetPath.toLowerCase().endsWith('.ts')) {
      throw new Error('save-target requires a TypeScript target file (.ts)');
    }
...
function cmdFeedback(sid: string, outputDir: string, asJson: boolean): void {
  const dir = sessionDir(sid, outputDir);
  const state = readJson<SiaState>(join(dir, 'state.json'));
  const target = readText(join(dir, `target-${state.iteration}.ts`));
  const spec = readText(join(dir, 'spec.md'));
...
function learningLoop(
  ...
  for (let i = 1; i <= maxIterations; i++) {
    ...
    const dir = sessionDir(sessionId, outputDir);
    const targetPath = join(dir, `target-${i}.ts`);
