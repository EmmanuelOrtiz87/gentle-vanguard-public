...
function trySymlink(target: string, linkPath: string): boolean {
  try {
    const result = runSync('cmd', ['/c', 'mklink', '/J', linkPath, target], { stdio: 'pipe' });
...
`;
  ...
  const cliPath = join(gfRoot, 'bin', 'gv.ts');
  const cliContent = `#!/usr/bin/env node
// gv.ts - Gentle-Vanguard CLI
...
async function main() {
    ...
    switch (cmd) {
        ...
        case 'validate': {
            console.log('Validating gentle-vanguard...');
        ...
        case 'update': {
            console.log('Skills sync was removed in Phase 1 cleanup. Skills are managed via .opencode/skills/ directory.');
            break;
        }
        default: {
            console.log('Gentle-Vanguard CLI');
            console.log('Usage: gv <command>');
            console.log('Commands: skills, validate, update');
        }
    }
}
