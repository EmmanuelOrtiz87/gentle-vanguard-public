...
function main(): void {
  ...
  if (process.platform === 'win32') {
    ...
    if (existsSync(taskScript)) {
      ...
      // while the user is logged on interactively. Registration is idempotent
      // (-Force) and migrates the old visible pwsh.exe form on existing installs.
      const psRegister = [
        '-Command',
        `$action = New-ScheduledTaskAction -Execute '${process.execPath}' -Argument '--import tsx "${taskScript}"' -WorkingDirectory '${root}'; ` +
      ...
      let regResult = runCmd('powershell', psRegister);
      if (regResult.status !== 0) {
        // S4U registration can be denied without elevation — fall back to a
        // wscript hidden wrapper (GUI-subsystem host, never allocates a console).
        const vbsPath = join(root, '.runtime', 'codegraph-sync-hidden.vbs');
        const vbsContent =
          `' Hidden launcher for Gentle-Vanguard-CodeGraph-Sync (generated by bootstrap)\n` +
          `CreateObject("Wscript.Shell").Run """${process.execPath}"" --import tsx ""${taskScript}""", 0, False\n`;
        ...
        regResult = runCmd('powershell', psFallback);
        if (regResult.status === 0) {
          writeSuccess(
            `Scheduled task '${taskName}' created hidden via wscript wrapper (syncs CodeGraph every hour).`,
          );
        } else {
          writeInfo(
        ...
        writeSuccess(
          `Scheduled task '${taskName}' registered hidden (node-direct, S4U principal).`,
        );
      }
    } else {
