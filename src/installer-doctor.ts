...
interface Check { id: string; status: Status; message: string; required: boolean }
...
function atLeast(actual: string | null, expected: string): boolean {
  const a = tuple(actual); const e = tuple(expected);
  if (!a || !e) return true;
...
function commandCheck(id: string, command: string, required: boolean, minimum?: string): Check {
  ...
  if (!version) return { id, status: required ? 'FAIL' : 'WARN', message: `${command} no está instalado`, required };
  if (minimum && !atLeast(version, minimum)) return { id, status: required ? 'FAIL' : 'WARN', message: `${version} < ${minimum}`, required };
...
function pathCheck(id: string, relativePath: string, required: boolean): Check {
  ...
  return { id, status: present ? 'PASS' : required ? 'FAIL' : 'WARN', message: present ? relativePath : `falta ${relativePath}`, required };
}

function main(): void {
  const manifestPath = join(root, 'config', 'installer-manifest.json');
  if (!existsSync(manifestPath)) { console.error('Falta config/installer-manifest.json'); process.exit(2); }
  const manifest = JSON.parse(readFileSync(manifestPath, 'utf8')) as {
    minimumNode: string; minimumPnpm: string;
  ...
  const checks: Check[] = [];
  for (const item of manifest.required) checks.push(commandCheck(item.id, item.command, true, item.id === 'node' ? manifest.minimumNode : item.id === 'pnpm' ? manifest.minimumPnpm : undefined));
  ...
  const report = { product: 'Gentle-Vanguard', root, status: failed.length === 0 ? 'ready' : 'incomplete', checks,
    next: failed.length === 0 ? 'Run npm run db:init and npm run watchtower:health.' : 'Instala las dependencias requeridas y ejecuta este comando otra vez.' };
  if (json) console.log(JSON.stringify(report, null, 2));
  else { console.log(`Gentle-Vanguard Installer Doctor — ${report.status.toUpperCase()}`); for (const check of checks) console.log(`[${check.status}] ${check.id}: ${check.message}`); console.log(`\nNext: ${report.next}`); }
