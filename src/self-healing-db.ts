...
async function getDBMetrics(): Promise<DBMetrics> {
  ...
  try {
    ...
    // Tables
    const tableResult = db.prepare("SELECT COUNT(*) as count FROM sqlite_master WHERE type='table'").get() as any;
    metrics.tableCount = tableResult?.count || 0;
    
    // Row count (aproximado)
    try {
      const rows = db.prepare("SELECT SUM((SELECT COUNT(*) FROM sqlite_master WHERE type='table')) as count").get() as any;
...
async function decideHealing(metrics: DBMetrics): Promise<Array<{ action: string; reason: string }>> {
